#Convert the below code to OOPs
import datetime
import sqlite3
...
class Main(tk.Tk):
    def __init__(self):
        ...
        self.self.sideFrame = Frame(self.root, background="#1b1b1b", width=250)
        self.self.buttonFrame = Frame(self.root, background="#212121", height=60)
        self.self.buttonFrame.pack(side=TOP, fill=X)
        self.self.sideFrame.pack(side=LEFT, fill=Y)
        ...
        def edit_expense(self):
            ...
            edit_btn = Button(self.buttonFrame, text='Edit expense', width=30, command=edit_existing_expense)
            edit_btn.pack()
        ...
        date = DateEntry(self.sideFrame, date=datetime.datetime.now())
        ...
        expense_list = ttk.OptionMenu(self.sideFrame, VoP,
        ...
        ttk.Entry(self.sideFrame, text=amnt).place(x=70, y=140)

        ttk.Label(self.sideFrame, text="Payee: ", background="#1b1b1b").place(x=5, y=205)
        ttk.Entry(self.sideFrame, text=payee).place(x=60, y=205)

        ttk.Label(self.sideFrame, text="Mode Of Pay: ", background="#1b1b1b").place(x=5, y=270)
        mode_of_pay = ttk.OptionMenu(self.sideFrame, MoP,
                                    *['Cash', 'Cheque', 'Credit Card', 'Debit Card', 'Paytm', 'Google Pay', 'Razorpay',
                                    'BHIM UPI'])
        mode_of_pay.config(width=10)
        mode_of_pay.place(x=100, y=265)

        ttk.Button(self.sideFrame, text='Add expense', command=add_another_expense).place(x=10, y=350)

        #########################################################

        ttk.Button(self.buttonFrame, text='Delete Expense', command=remove_expense).place(x=5, y=11)

        ttk.Button(self.buttonFrame, text='Clear Fields in DataEntry', command=clear_fields).place(x=130, y=11)

        ttk.Button(self.buttonFrame, text='Delete All Expenses',
                command=remove_all_expenses).place(x=310, y=11)

        ttk.Button(self.buttonFrame, text='Edit Selected Expense', command=edit_expense).place(x=465, y=11)

        ttk.Button(self.buttonFrame, text='Convert Expense to a sentence', command=selected_expense_to_words).place(x=630, y=11)

        tree_frame = Frame(self.root, background="#121212")
